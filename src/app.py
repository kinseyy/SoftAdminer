...
class Logic():
    def __init__(self):
        ...
        self.github_dialog = QCoreApplication.translate("MessageBox", "Visit the SoftAdminer GitHub page?")
        self.quit_dialog = QCoreApplication.translate("MessageBox", "Quit SoftAdminer?")
    ...
    def show_updates(self, latest_version):
        ...
        if version.parse(latest_version) > version.parse(__version__):
            msg_update = QCoreApplication.translate("MessageBox", "SoftAdminer {0} is available.\n\nVisit download page?").format(latest_version)
            if self.message_box(msg_update, 2) == QMessageBox.Yes:
                webbrowser.open_new('https://github.com/kinseyy')
...
class CheckUpdates(QThread):
    ...
    def run(self):
        try:
            api_url = 'https://api.github.com/repos/kinseyy/SoftAdminer/releases/latest'
